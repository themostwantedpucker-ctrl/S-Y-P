              ...
              <div className="flex gap-2">
                <Button
                  onClick={() => {
                    ...
                    if (!u.toLowerCase().endsWith('/api')) u = `${u}/api`;
                    setConfiguredApiBaseUrl(u);
                    setServerUrl(u);
                    toast({ title: 'Server URL saved', description: `Using: ${u}` });
                  }}
                >Save URL</Button>
                <Button
                  variant="outline"
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (ct.includes('application/json')) {
                        ...
                        setHealth(`OK (${res.status}): ${json.status || 'OK'}`);
                        toast({ title: 'Health check OK', description: 'Server responded successfully.' });
                      } else {
                        const text = await res.text();
                        const preview = text.slice(0, 100).replace(/\s+/g, ' ');
                        setHealth(`Failed (${res.status}): Non-JSON response. Preview: ${preview}`);
                        toast({ title: 'Health check failed', description: 'Received non-JSON response. Check your API base URL ends with /api.', variant: 'destructive' });
                      }
                    } catch (e) {
...
const AutoSyncRestoreToggles: React.FC = () => {
  ...
  const handleToggleSync = async () => {
    if (!autoSync) {
      ...
      toast({ title: 'Auto Sync enabled', description: 'Your data will now be backed up every 60 seconds.' });
    } else {
      setAutoSync(false);
  ...
  const handleToggleRestore = async () => {
    if (!autoRestore) {
      ...
      toast({ title: 'Auto Restore enabled', description: 'Your data will now be restored every 60 seconds.' });
    } else {
